...
const toolNames: Record<string, string> = {
  headings: 'Headings Extractor',
  links: 'Links Extractor',
  images: 'Images Extractor',
  videos: 'Videos Extractor',
  fonts: 'Fonts Detector',
  colors: 'Color Palette',
  emails: 'Email Extractor',
  phones: 'Phone Number Extractor',
  robots: 'Robots.txt Viewer',
  meta: 'Meta Tags Extractor',
  resources: 'Resources Extractor',
...
export default function ToolPage() {
  ...
  const [activeTab, setActiveTab] = useState<string>(toolId || 'overview');
  ...
  const handleExtract = async (url: string) => {
    ...
    try {
      ...
      if (result.success) {
        setData(result.data);
        // Auto-scroll to the specific tab if tool is selected
        if (toolId && toolId !== 'overview') {
          setTimeout(() => {
            setActiveTab(toolId);
          }, 100);
  ...
  const toolName = toolNames[toolId] || 'Website Extractor';
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="text-center mb-12">
            ...
            <p className="text-xl text-gray-600 dark:text-gray-300 max-w-2xl mx-auto">
              Extract {toolId} data from any website URL
            </p>
          </div>
